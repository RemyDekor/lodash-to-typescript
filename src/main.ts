...
const loFilterNullableRes = _.filter(n => (n % 2 === 0))(_.get('arrNullable', loObj));  // 👻 Accepts `null` (outputs `[]` with unknown[] type)
const reFilterNullableResJs = Re.filter(n => (n % 2 === 0))(reObj?.arrNullable); // 🛑 Does not accept nullable, but warns with Typescript
const reFilterNullableResTs = Re.filter(reObj?.arrNullable, n => (n % 2 === 0)); // 🛑 Does not accept nullable, but warns with Typescript
const rbFilterNullableRes = Rb.filter(n => n % 2 === 0)(rbObj?.arrNullable); // 🛑 Does not accept nullable, but warns with Typescript
